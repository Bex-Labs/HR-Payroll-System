...
function cacheDomElements() {
  state.dom = {
    ...
    // Self-Service workspace tab and section for HR staff to manage their own
    ...
    // EMPLOYEE SELF-SERVICE - HR
    hrSelfServiceSection: document.getElementById("hrSelfServiceSection"),
...
function bindEvents() {
  ...
  // EMPLOYEE SELF-SERVICE - HR
  // HR staff open their own employee self-service workspace (leave + payroll).
...
function switchHrWorkspace(workspace) {
  ...
  // Four core workspaces + Self-Service:
  ...
  // Self-Service = HR staff's own leave and payroll self-service (employee view)
...
// EMPLOYEE SELF-SERVICE - HR
// Lazily initialises the self-service module on the first time the HR user
// opens the Self-Service tab. Subsequent clicks only remember/switch the workspace;
// data is already loaded so no duplicate queries are made.
...
async function generateNextEmployeeCustomId() {
  ...
  // Employee Number is generated by a Supabase RPC that now accepts a
  // tenant_id so each company maintains its own independent counter.
  // Two HR users from different companies will never receive the same
  // staff number, and each company's sequence always starts from P1.
  const supabase = getSupabaseClient();

  // Resolve the current company's tenant ID.  This throws if no tenant
  // context is available, which prevents a number being generated without
  // a valid company scope.
  const tenantId = getRequiredTenantIdForHrEmployeeData();
